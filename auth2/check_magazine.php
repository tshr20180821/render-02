...
__HEREDOC__;
    ...
    if ($pdo_sqlite->query($sql_select)->fetchColumn() != 0) {
        $options = [
            CURLOPT_USERPWD => base64_encode($_ENV['BASIC_USER'] . ':' . $_ENV['BASIC_PASSWORD']),
        ];
...
UPDATE m_magazine_data
   SET bibid = :b_bibid
      ,reserve = 1
      ,update_datetime = LOCALTIMESTAMP + interval '9 hours'
      ,check_datetime = LOCALTIMESTAMP + interval '9 hours'
 WHERE symbol = :b_symbol
   AND title = :b_title
__HEREDOC__;
...
__HEREDOC__;
    ...
    $log->info('PostgreSQL Version : ' . $pdo->query('SELECT version()')->fetchColumn());
